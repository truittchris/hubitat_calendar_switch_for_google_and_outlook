...
+@Field static final String DRIVER_VERSION = "1.0.6"
 ...
 command "applyRulesNow" command "fetchNow" command "fetchNowAndApply" command "testNow"
...
+// NOTE: Driver preferences are input-only. Do not use section()/paragraph() and do not use app-only UI helpers.
+preferences {
...
+}
 ...
 Fetch now - asks the parent app to fetch new events.
 Fetch now and apply - fetches and evaluates.
 Test now - fetches and evaluates, sets Last Test Result.\n\nClear match words - clears Match words.
 Clear ignore words - clears Ignore words.
 Clear both - clears both word lists."
 device.updateDataValue("commandHelp", s) sendEvent(name: "commandHelp", value: s) }
+
+void clearMatchWords() {
+    device.updateSetting("includeKeywords", [value: "", type: "text"])
+    if (state?.lastEvents instanceof List && !((List)state.lastEvents).isEmpty()) reEvaluateFromCache()
+}
+
...
+    if (v instanceof Map) {
+        def dt = v.dateTime ?: v.datetime ?: null
...
+    // Graph may send Windows TZ names; ZoneId.of() will throw. Fall back to hub tz.
+    if (tzId != null && tzId.toString().trim()) {
+        try {
+            return ZoneId.of(tzId.toString().trim())
+        } catch (Exception ignored) {
+            // fall through to hub tz
+        }
+    }
+    try {
